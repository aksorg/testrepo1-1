...
module "keyvault" {
  ...
  count                           = var.keyvault_name == "" && var.certificate_data != "" ? 1 : 0
  
  source                          = "git::https://github.com/DigitalInnovation/cloud-platform-automation-assets.git//modules/Keyvault?ref=app-gateway"
...
resource "azurerm_application_gateway" "app_gateway" {
  ...
  ssl_certificate {
    ...
    data     = var.keyvault_name != "" ? data.azurerm_key_vault_secret.secret[0].value : var.certificate_data
    password = var.keyvault_name != "" ? data.azurerm_key_vault_secret.secretpass[0].value : var.certificate_pass
  }
